...
use super::input::{HostCommand, HostCommandMap, InputFrontend, PollResult};
...
pub fn run(
    ...
    let initial_path = initial.as_ref().map(|l| l.path.clone());
...
impl App {
    ...
    fn present_frame(&mut self, timed: bool) -> Result<(Duration, Duration, Duration), String> {
        ...
        let game_rgba: Option<Vec<u8>> = self
            .rom_loaded
            .then(|| self.renderer.as_ref().unwrap().presented_rgba().to_vec());
    ...
    fn refresh_debug_monitor_ex(&mut self, force: bool) {
        ...
        let (machine, snap_cost) = if want_machine {
            let t_snap = Instant::now();
            // Non-blocking: runtime publishes into a latest-debug mailbox.
            let m = self.runtime.take_debug_snapshot();
            (m, t_snap.elapsed())
        } else {
            (None, Duration::ZERO)
        };
        ...
        if let Some(mon) = self.debug_monitor.as_mut() {
            mon.update_cache(machine, host, pending, remaining, has_report, last_report);
            mon.maybe_request_redraw();
        }
    }
    ...
    fn arm_capture(&mut self) {
        ...
        let machine = self.runtime.take_debug_snapshot();
        self.diagnostics.arm_capture(title, path, machine);
    ...
    fn copy_snapshot(&mut self) {
        ...
        let report = build_snapshot_report(&title, &path, &host, machine.as_ref());
    ...
    fn pick_rom(&self) -> Option<PathBuf> {
        let mut dialog = rfd::FileDialog::new()
            .add_filter("Game Boy ROM", rom_dialog_extensions())
            .set_title("Open ROM");
    ...
    fn load_rom_path(&mut self, path: &Path) -> Result<(), String> {
        ...
        if self.verbosity != Verbosity::Quiet {
            ...
            if opened.cart.has_battery_backed_ram() {
    ...
    fn install_opened(&mut self, opened: OpenedRom) -> Result<(), String> {
        ...
        self.settings.save();
        self.cached_rom_path = path.clone();
        self.rom_loaded = true;
        self.input.reset_edges();
        self.runtime.send(EmuCommand::LoadRom {
            path,
            save_path,
            title,
            cart: Box::new(cart),
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if self.verbosity != Verbosity::Quiet {
            ...
            println!(
                "keys: arrows=D-pad  Z=B  X=A  Backspace=Select  Enter=Start  F5=QuickSave  F8=QuickLoad  F6=Screenshot  hold R=Rewind  F11=Fullscreen  F12=Monitor  F9=Capture"
            );
        }
