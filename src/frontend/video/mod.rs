...
/// Default integer scale factor for the 160×144 LCD.
...
impl Renderer {
    /// Create the presenter. `vsync` should stay **false** when [`FramePacer`]
    /// owns wall-clock cadence (otherwise GPU wait + sleep = half-speed).
    pub fn new(window: Arc<Window>, title: &str, vsync: bool) -> Result<Self, String> {
        ...
        let pixels = PixelsBuilder::new(SCREEN_WIDTH as u32, SCREEN_HEIGHT as u32, surface)
            .enable_vsync(vsync)
            .build()
