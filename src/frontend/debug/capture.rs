...
impl DiagnosticRing {
    ...
    pub fn arm_capture(&mut self, title: String, rom_path: String, machine: Option<MachineDebug>) {
...
fn build_report(cap: &CaptureState, ring: &DiagnosticRing) -> String {
    ...
    if let Some(m) = &cap.machine {
        out.push_str("## MACHINE SNAPSHOT (at arm)\n\n");
        out.push_str(&format!(
            "PC={:04X} SP={:04X} AF={:04X} BC={:04X} DE={:04X} HL={:04X} IME={} HALT={}\n",
            m.cpu.pc, m.cpu.sp, m.cpu.af, m.cpu.bc, m.cpu.de, m.cpu.hl, m.cpu.ime, m.cpu.halted
        ));
        out.push_str(&format!(
        ...
        out.push_str(&format!(
            ...
            m.apu.nr52, m.apu.ch1.active, m.apu.ch2.active, m.apu.ch3.active, m.apu.ch4.active
        ));
        out.push_str(&format!(
