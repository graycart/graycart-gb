//! In-memory `MachineStateV1` ring for hold-to-rewind (not postcard/disk).
...
impl RewindRing {
    pub fn new(template: &MachineStateV1) -> Self {
        Self {
            slots: vec![template.clone()],
            head: 0,
            len: 0,
            bytes: 0,
            scrub_idx: None,
    ...
    #[allow(dead_code)] // public API; exercised in unit tests
    ...
    pub fn push(&mut self, state: MachineStateV1) {
    ...
    pub fn scrub_back(&mut self) -> Option<&MachineStateV1> {
    ...
    fn newest_pc(&self) -> u16 {
        if self.len == 0 {
            ...
            let cap = self.slots.len();
            self.slots[(self.head + self.len - 1) % cap].cpu.pc
        }
    }
}

#[allow(dead_code)] // UI reads settings.rewind_enabled directly; emu thread owns rewind ring
