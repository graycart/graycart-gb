...
impl Gui {
    ...
    pub fn prepare(
        ...
        self.sync_game_texture(game_rgba, settings.display_mode);
            ...
            egui::CentralPanel::default()
                ...
                .show_inside(ui, |ui| {
                    if let Some(tex) = game_tex.as_ref() {
                        ...
                        let (w, h) =
                            game_image_size(avail.x, avail.y, integer_scaling, display_mode);
                        if w > 0.0 && h > 0.0 {
    ...
    fn sync_game_texture(&mut self, game_rgba: Option<&[u8]>, mode: DisplayMode) {
        let Some(rgba) = game_rgba else {
            self.game_texture = None;
            return;
        };
        let expected = SCREEN_WIDTH * SCREEN_HEIGHT * 4;
        ...
        match self.game_texture.as_mut() {
            ...
            None => {
                self.game_texture =
                    Some(self.egui_ctx.load_texture("gb_framebuffer", image, filter));
            }
        }
    }
